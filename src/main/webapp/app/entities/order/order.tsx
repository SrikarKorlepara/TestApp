  ...
  return (
    <div>
      <h2 id="order-heading" data-cy="OrderHeading">
        <Translate contentKey="ecommerceApp.order.home.title">Orders</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ecommerceApp.order.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ecommerceApp.order.home.createLabel">Create new Order</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderList && orderList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="ecommerceApp.order.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('orderDate')}>
                  <Translate contentKey="ecommerceApp.order.orderDate">Order Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('orderDate')} />
                </th>
                <th className="hand" onClick={sort('status')}>
                  <Translate contentKey="ecommerceApp.order.status">Status</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('status')} />
                </th>
                <th className="hand" onClick={sort('totalAmount')}>
                  <Translate contentKey="ecommerceApp.order.totalAmount">Total Amount</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('totalAmount')} />
                </th>
                <th>
                  <Translate contentKey="ecommerceApp.order.customer">Customer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ecommerceApp.order.home.notFound">No Orders found</Translate>
            </div>
          )
        )}
      </div>
